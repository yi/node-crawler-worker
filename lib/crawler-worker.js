...
(function() {
  var fs, jobs, kue, logger, mkdirp, p, path, redis, _;
  ...
  p.version('0.0.1').option('-p, --port <n>', 'redis server port').option('-h, --host [VALUE]', 'redis server host').option('-g, --gameserver-id [VALUE]', 'game server id, for which this crawler is working').option('-o, --output [PATH]', 'file output path').parse(process.argv);
  ...
  kue.redis.createClient = function() {
    ...
    client.on("error", function(error) {
      return console.error("[redis.on_error] " + error);
    });
    client.on("end", function() {
      return console.warn("[db.on_end]");
    });
    client.on("ready", function() {
      return console.info("[db] redis client is ready to server.");
    });
    client.on("reconnecting", function(info) {
      return console.info("[db] redis client is reconnecting to datastore:" + p.host + ":" + p.port + "... delay:" + info.delay + ", attempt:" + info.attempt);
    });
    return client;
  };
  ...
  process.on('uncaughtException', function(error) {
    return console.log("[crawler-worker::uncaughtException] " + error + ", stack:" + error.stack);
  });
